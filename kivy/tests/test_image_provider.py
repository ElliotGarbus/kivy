...
class TestAvailableProviders:
    ...
    def test_available_providers_lowercase(self):
        ...
        providers = CoreImage.available_providers()
        for provider in providers:
            assert provider == provider.lower(), f"Provider {provider!r} should be lowercase"
...
class TestImageProviderURIScheme:
    ...
    def test_uri_scheme_regex_pattern(self):
        ...
        valid_uris = [
            ...
            ('@image_provider:imageio(../images/icon.png)', 'imageio', '../images/icon.png'),
            ('@image_provider:PIL(image.png)', 'PIL', 'image.png'),
        ]
...
class TestInvalidProviderHandling:
    ...
    def test_invalid_provider_lenient_mode(self, test_image_path, kivy_window, monkeypatch):
        ...
        img = ImageLoader.load(test_image_path, image_provider='nonexistent_provider_xyz')
        # In lenient mode, it should fallback to default providers and succeed
        assert img is not None

    def test_invalid_provider_strict_mode(self, test_image_path, kivy_window, monkeypatch):
